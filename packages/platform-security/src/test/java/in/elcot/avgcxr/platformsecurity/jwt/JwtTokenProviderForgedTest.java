 ...
 *
 * <p>Before the fix, {@code validateToken} caught {@code java.lang.SecurityException} instead of
 * {@code io.jsonwebtoken.JwtException}, so a tampered/forged token threw {@code
 * io.jsonwebtoken.security.SignatureException} out of the method (HTTP 500) rather than being
 * rejected. These tests assert clean rejection (return false), and that genuinely valid tokens
 * still pass.
 */
class JwtTokenProviderForgedTest {
  ...
  void rejectsForgedToken() {
    ...
    String forged =
        "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJoYWNrZXIifQ.bogus_signature_xxxxxxxxxxxxxxxxxx";
