...
/**
 * Type-safe client for Strapi v4 REST API. Handles bilingual content retrieval, media URL
 * resolution, and error mapping.
 *
 * <p>Used by bounded contexts to fetch CMS-managed content such as scheme descriptions, FAQs, news
 * articles, and banner images.
 */
@Component
public class StrapiClient {
  ...
  public <T> StrapiResponse<T> get(String path, Class<T> responseType, Map<String, String> params) {
    ...
    if (!response.getStatusCode().is2xxSuccessful()) {
      throw new StrapiClientException(
          "Strapi API error: " + response.getStatusCode(), response.getStatusCode().value());
    }
    ...
    try {
      return objectMapper.readValue(
          ...
          objectMapper
              .getTypeFactory()
              .constructParametricType(StrapiResponse.class, responseType));
    } catch (Exception e) {
   ...
   */
  public <T> StrapiResponse<T> getById(
      String path, int id, Class<T> responseType, Map<String, String> params) {
    return get(path + "/" + id, responseType, params);
  }

  /**
   * Resolve media URL to absolute URL. Converts relative Strapi media paths to full URLs.
   *
