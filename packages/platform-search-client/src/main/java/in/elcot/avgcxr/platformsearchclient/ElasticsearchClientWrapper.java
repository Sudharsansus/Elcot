...
/**
 * Type-safe wrapper around the Elasticsearch Java client.
 * Provides search, index, and delete operations for AVGC-XR Portal documents.
 *
 * <p>Supports bilingual search with Tamil and English analyzers,
 * fuzzy matching, and faceted search via aggregations.</p>
 */
@Component
public class ElasticsearchClientWrapper {
     ...
     */
    public <T> SearchResult<T> search(String indexName, Class<T> documentType,
                                        String queryString, int from, int size) {
        try {
            ...
            log.error("Elasticsearch search failed for index: {}, query: {}, error: {}",
                    indexName, queryString, e.getMessage());
    ...
    public <T> void index(String indexName, String documentId, T document) {
        try {
            ...
            log.error("Failed to index document: {}/{}, error: {}", indexName, documentId, e.getMessage());
    ...
    public void delete(String indexName, String documentId) {
        try {
            ...
            log.error("Failed to delete document: {}/{}, error: {}", indexName, documentId, e.getMessage());
    ...
    private <T> SearchResult<T> mapToSearchResult(SearchResponse<T> response, int from, int size) {
        ...
        List<SearchResult.SearchHit<T>> hits = response.hits().hits().stream()
                .map(this::mapHit)
    ...
    private <T> SearchResult.SearchHit<T> mapHit(Hit<T> hit) {
        ...
        if (hit.highlight() != null) {
            Map<String, List<String>> highlightMap = hit.highlight().entrySet().stream()
