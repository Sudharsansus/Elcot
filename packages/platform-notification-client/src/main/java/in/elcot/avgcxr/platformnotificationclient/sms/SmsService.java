...
/**
 * SMS delivery service for AVGC-XR Portal. Integrates with the NIC SMS gateway used by Tamil Nadu
 * government departments.
 *
 * <p>Supports bilingual SMS messages and OTP delivery. All SMS content must comply with TRAI DND
 * regulations and government communication guidelines.
 */
@Service
public class SmsService {
  ...
  public void sendSms(String mobileNumber, String message, String templateId) {
    ...
    try {
      ...
      if (response.getStatusCode().is2xxSuccessful()) {
        log.info(
            "SMS sent successfully to: {}, templateId: {}", maskMobile(mobileNumber), templateId);
      } else {
        log.error(
            "SMS delivery failed to: {}, status: {}, body: {}",
  ...
  public void sendOtp(String mobileNumber, String otp, int validityMinutes) {
    String message =
        String.format(
            "Your AVGC-XR Portal OTP is %s. Valid for %d minutes. "
  ...
  private void validateMobileNumber(String mobileNumber) {
    if (mobileNumber == null || !mobileNumber.matches("^[6-9]\\d{9}$")) {
      throw new IllegalArgumentException(
          "Invalid Indian mobile number: " + maskMobile(mobileNumber));
    }
  }
  ...
  private String maskMobile(String mobileNumber) {
    ...
    return mobileNumber.substring(0, 2)
        + "******"
...
/** Immutable request object for SMS notifications. (Definition in SmsRequest.java) */
