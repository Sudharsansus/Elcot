 ...
 *
 * <p>When PgBouncer is used in transaction pooling mode, prepared statements
 * must be disabled (prepareThreshold=0) because PgBouncer does not support
 * prepared statement proxying in transaction mode.</p>
 *
 * <p>Activated by the "pgbouncer" Spring profile (default for staging/prod).</p>
 */
@Configuration
@Profile("pgbouncer")
