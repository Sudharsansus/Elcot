...
**Purpose:** Folder holding the files listed below (part of `apps/applicant-portal/src`).

## Files (7)

| File | Type | What it does |
| --- | --- | --- |
| `app.component.html` | HTML | HTML template or page. |
| `app.component.scss` | Sass | PORTAL SHELL — glass violet header + deep-purple footer |
| `app.component.spec.ts` | TypeScript | Unit test specification for the matching source file. |
| `app.component.ts` | TypeScript | App — UI component (template + logic for a screen or widget). |
| `app.config.server.ts` | TypeScript | TypeScript source. |
| `app.config.ts` | TypeScript | App configuration. |
| `app.routes.ts` | TypeScript | Route definitions for the App area (lazy-loaded screens). |
