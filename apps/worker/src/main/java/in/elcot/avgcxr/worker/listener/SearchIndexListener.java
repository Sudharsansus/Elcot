...
/**
 * Consumes {@code avgc.search.index} events. Calls the API to upsert
 * the document into the Elasticsearch index.
 *
 * <p>Replaces the previous log-only stub. The API has the real
 * Elasticsearch client and handles the index lifecycle (create index,
 * upsert document, refresh).</p>
 */
@Component
public class SearchIndexListener {
    ...
    public void handleSearchIndex(Map<String, Object> payload) {
        try {
            ...
            String index = payload != null && payload.get("index") != null
                    ? payload.get("index").toString() : "avgcxr-general";
            String id = payload != null && payload.get("id") != null
                    ? payload.get("id").toString() : null;

            String url = apiBaseUrl + "/api/v1/search/index/" + index;
            if (id != null) url += "/" + id;

            ResponseEntity<Map> resp = restTemplate.exchange(
                    url, HttpMethod.PUT, new HttpEntity<>(payload, h), Map.class
            );
            ...
            log.error("ES indexing failed for payload id={}",
                    payload != null ? payload.get("id") : "null", e);
