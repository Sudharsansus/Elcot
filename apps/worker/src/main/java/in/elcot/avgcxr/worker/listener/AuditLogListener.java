...
/**
 * Consumes {@code avgc.audit.created} events. Calls the API to persist the audit record in the
 * partitioned {@code audit_log} table.
 *
 * <p>Replaces the previous log-only stub. The audit data is parsed from JSON and forwarded to
 * {@code POST /api/v1/audit-logs} which the AuditLogController handles with a real JPA INSERT.
 */
@Component
public class AuditLogListener {
  ...
  public void handleAuditLog(Map<String, Object> payload) {
    try {
      ...
      log.error(
          "Audit log persistence failed for payload: {}",
