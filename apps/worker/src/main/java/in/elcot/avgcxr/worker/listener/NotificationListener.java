...
/**
 * Consumes {@code avgc.notification.dispatch} events and routes to email/SMS/push channels based on
 * the {@code channel} field.
 *
 * <p>Replaces the previous log-only stub that sent a hardcoded {@code recipient@example.com} email.
 * Now the actual recipient and channel from the event payload are used.
 */
@Component
public class NotificationListener {
  ...
  public void handleNotification(Map<String, Object> payload) {
    String channel =
        payload != null && payload.get("channel") != null
            ? payload.get("channel").toString().toUpperCase()
            : "EMAIL";
    String eventType =
        payload != null && payload.get("eventType") != null
            ? payload.get("eventType").toString()
            : "GENERIC";
    ...
    try {
      ...
      ResponseEntity<Map> resp =
          restTemplate.exchange(
              apiBaseUrl + endpoint, HttpMethod.POST, new HttpEntity<>(payload, h), Map.class);
