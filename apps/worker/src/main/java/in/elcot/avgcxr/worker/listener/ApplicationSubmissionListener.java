...
/**
 * Consumes {@code avgc.application.submitted} events. Calls the API to:
 *  1. Validate the application (404 if missing)
 *  2. Start a Flowable workflow instance
 *  3. Send a confirmation email via the notification queue
 *
 * <p>This is the real implementation that replaces the previous log-only
 * stub. It calls the live API service via HTTP so the worker stays
 * stateless and lightweight.</p>
 */
@Component
public class ApplicationSubmissionListener {
    ...
    public void handleApplicationSubmission(Map<String, Object> payload) {
        ...
        String applicationId = payload != null && payload.get("applicationId") != null
                ? payload.get("applicationId").toString() : "unknown";
        ...
        try {
            ...
            ResponseEntity<Map> startResp = restTemplate.exchange(
                    ...
                    HttpMethod.POST,
                    new HttpEntity<>(Map.of("processKey", "applicationReview",
                            "variables", Map.of("applicationId", applicationId)), h),
                    Map.class
            );
            if (startResp.getStatusCode().is2xxSuccessful()) {
                log.info("Workflow started for application {}", applicationId);
            } else {
                log.warn("Workflow start returned {} for application {}",
                        startResp.getStatusCode(), applicationId);
            }
            ...
            restTemplate.exchange(
                    ...
                    new HttpEntity<>(Map.of(
                            ...
                            "body", "Your application " + applicationId + " is under review."),
                            h),
                    Map.class
            );
            log.info("Application {} processed in {}ms", applicationId, System.currentTimeMillis() - start);
        } catch (Exception e) {
