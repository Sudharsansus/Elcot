 ...
 *
 * <p>Uses Mockito-mocked repository ports (the project has no embedded test DB), with the REAL
 * {@link RuleBasedLlmProvider}, {@link PromptBuilder} and {@link TamilSupport} so the bilingual
 * orchestration is genuinely exercised.
 */
class ChatServiceTest {

  private ChatService newService(
      ChatSessionRepositoryPort sessionRepo, ChatMessageRepositoryPort messageRepo) {
  ...
  void sendMessageCreatesSessionPersistsBothMessagesAndReplies() {
    ...
    when(messageRepo.findRecentBySessionId(any(), anyInt()))
        .thenReturn(
            List.of(
                ChatMessage.userMessage(UUID.randomUUID(), "What schemes are available?", "en")));
    ...
    assertFalse(
        result.assistantMessage().getContent().isBlank(), "assistant reply must not be blank");
  ...
  void ruleBasedProviderAnswersEnglishQuery() {
    ...
    assertTrue(
        r.content().toLowerCase().contains("scheme"),
  ...
  void ruleBasedProviderRepliesWithTamilScript() {
    ...
    LlmProvider.LlmResponse r =
        p.complete(
            "sys", List.of(PromptBuilder.msg("user", "tamil")), LlmProvider.LlmOptions.defaults());
    assertTrue(
        r.content().codePoints().anyMatch(cp -> cp >= 0x0B80 && cp <= 0x0BFF),
