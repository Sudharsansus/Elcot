...
class UserTest {
  ...
  class Registration {
    ...
    void activatesOnActivate() {
      var user =
          new User(UserId.generate(), "u", "u@x.com", "+91", "U", java.util.Set.of("PUBLIC"));
      user.activate();
  ...
  class LoginTracking {
    ...
    void recordLoginUpdatesTimestamp() {
      ...
      assertThat(user.getLastLoginAt())
          .isNotNull()
          .isBetween(
              java.time.Instant.now().minusSeconds(5), java.time.Instant.now().plusSeconds(1));
    }
  }
  ...
  class ProfileUpdates {
    ...
    void updateProfile() {
      var user =
          new User(UserId.generate(), "u", "u@x.com", "+91", "Old", java.util.Set.of("PUBLIC"));
      user.updateProfile(
          "New Name", "\u0baa\u0bc1\u0ba4\u0bbf\u0baf \u0baa\u0bc6\u0baf\u0bb0\u0bcd", "Chennai");
      assertThat(user.getFullName()).isEqualTo("New Name");
      assertThat(user.getFullNameTamil())
          .isEqualTo("\u0baa\u0bc1\u0ba4\u0bbf\u0baf \u0baa\u0bc6\u0baf\u0bb0\u0bcd");
