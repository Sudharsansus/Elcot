...
/**
 * File service: persists metadata via FileRepositoryPort AND uploads the
 * byte payload via FileStoragePort (MinIO). Both outputs are required
 * for the tender document-upload requirement.
 */
@Service
...
public class FileService implements CreateFileUseCase, GetFileUseCase {
    ...
    public File create(CreateFileCommand cmd) {
        ...
        log.info("Uploading file {} ({} bytes) to object storage", cmd.originalFileName(), cmd.fileSize());
    ...
    public File getById(FileId id) {
        return repository.findById(id)
                .orElseThrow(() -> new FileNotFoundException(id.value()));
    }
}
