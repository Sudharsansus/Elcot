...
public class DashboardRepositoryAdapter implements DashboardRepositoryPort {
  ...
  public DashboardRepositoryAdapter(EntityManager em) {
    this.em = em;
  }
  ...
  public Map<String, Long> getApplicationsBySubSector() {
    Query q =
        em.createNativeQuery("SELECT sub_sector, COUNT(*) FROM applications GROUP BY sub_sector");
    return toMap(q.getResultList());
  }

  @Override
  public long countTotalApplications() {
    return ((Number) em.createNativeQuery("SELECT COUNT(*) FROM applications").getSingleResult())
        .longValue();
  }
  ...
  public long countPendingReviews() {
    return ((Number)
            em.createNativeQuery("SELECT COUNT(*) FROM applications WHERE status = 'UNDER_REVIEW'")
                .getSingleResult())
        .longValue();
  }
  ...
  public long countApprovedToday() {
    return ((Number)
            em.createNativeQuery(
                    "SELECT COUNT(*) FROM applications WHERE status = 'APPROVED' AND DATE(updated_at) = CURRENT_DATE")
                .getSingleResult())
        .longValue();
  }

  @Override
  public long sumTotalDisbursed() {
    return ((Number)
            em.createNativeQuery(
                    "SELECT COALESCE(SUM(approved_amount), 0) FROM applications WHERE status = 'DISBURSED'")
                .getSingleResult())
        .longValue();
  }
