...
/**
 * Real password reset flow: 1. requestReset(email) → generate 32-byte token, store SHA-256 hash in
 * DB with 1-hour expiry, publish reset link to the notification queue 2. resetPassword(token,
 * newPassword) → verify hash + expiry + not-used, bcrypt-hash new password, UPDATE
 * users.password_hash, mark token used
 */
@Service
public class PasswordResetService {
  ...
  /**
   * Generate a reset token. If email matches a user, store hashed token in DB and publish reset
   * link to notification queue. Always returns the same shape (no user enumeration leak).
   */
  @Transactional
  public void requestReset(String email) {
    ...
    try {
      ...
      java.util.List<UUID> ids =
          em.createQuery("SELECT u.id FROM UserEntity u WHERE u.email = :email")
    ...
    if (userId != null) {
      ...
      if (rt != null) {
        rt.convertAndSend(RabbitMQConfig.EXCHANGE, RabbitMQConfig.RK_NOTIFICATION_DISPATCH, event);
      } else {
  ...
  public void resetPassword(String token, String newPassword) {
    ...
    int updated =
        em.createQuery("UPDATE UserEntity u SET u.passwordHash = :hash WHERE u.id = :id")
