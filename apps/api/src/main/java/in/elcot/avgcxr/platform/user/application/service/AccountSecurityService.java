 ...
 *
 * <p>Brute-force protection (HIGH-011 audit fix): After MAX_FAILED_ATTEMPTS failed logins, the
 * account is locked for LOCKOUT_DURATION_MINUTES. Successful login resets the counter and clears
 * any lockout.
 *
 * <p>DPDP consent (HIGH-012 audit fix): Records explicit consent for data processing under India
 * DPDP Act 2023. Each consent row is immutable; revocation creates a new row with revoked_at set.
 */
@Service
@Transactional
...
public class AccountSecurityService {
  ...
  /**
   * Called from UserService.authenticate() after a failed password check. Increments counter, locks
   * the account if threshold exceeded.
   */
  public User recordFailedLogin(User user) {
    ...
    if (attempts >= maxFailedAttempts) {
      ...
      log.error(
          "Account LOCKED: user={} attempts={} until={}",
          user.getEmail(),
          attempts,
          user.getLockedUntil());
    } else {
      log.warn(
          "Failed login: user={} attempts={}/{}", user.getEmail(), attempts, maxFailedAttempts);
    }
  ...
  /**
   * Records a consent decision. Returns the persisted record. If a record already exists for (user,
   * type, policy_version) it's a no-op (idempotent).
   */
  public UserConsent recordConsent(
      UUID userId, String consentType, boolean granted, String ipAddress, String userAgent) {
    if (!userRepo.findById(UserId.of(userId.toString())).isPresent()) {
      throw new IllegalArgumentException("User not found: " + userId);
    }
    if (consentRepo.existsByUserIdAndConsentTypeAndPolicyVersion(
        userId, consentType, defaultPolicyVersion)) {
      log.debug(
          "Consent already recorded: user={} type={} policy={}",
          userId,
          consentType,
    ...
    log.info(
        "Consent recorded: user={} type={} granted={} policy={}",
        userId,
        consentType,
        granted,
        defaultPolicyVersion);
  ...
  /**
   * Records multiple consents at once (e.g., during registration). Used by CreateUserUseCaseService
   * to honor DPDP checkbox on signup.
   */
  public List<UserConsent> recordConsents(
      UUID userId, Map<String, Boolean> consents, String ipAddress, String userAgent) {
    ...
    return consents.entrySet().stream()
        .map(
            e ->
                recordConsent(
                    userId, e.getKey(), Boolean.TRUE.equals(e.getValue()), ipAddress, userAgent))
        .toList();
  }
