 ...
 *
 * <ul>
 *   <li>SLA tracking thresholds (priority-based response/resolution times)
 *   <li>Auto-escalation scheduler (escalates tickets exceeding SLA)
 *   <li>Ticket counter for monitoring
 * </ul>
 */
@Configuration
@EnableScheduling
public class HelpdeskConfig {
  ...
  /**
   * Run every 15 minutes: scan for tickets past SLA and escalate. In production this would publish
   * escalation events to RabbitMQ and the WorkflowService would update the ticket state.
   */
  @Scheduled(
      fixedDelayString = "${avgcxr.helpdesk.escalation-interval-ms:900000}",
  ...
  public record SlaThresholds(
      int highPriorityHours, int mediumPriorityHours, int lowPriorityHours) {
    public Duration slaFor(String priority) {
      int h =
          switch (priority == null ? "MEDIUM" : priority.toUpperCase()) {
  ...
  public static class TicketMetrics {
    ...
    public int getTotalEscalated() {
      return totalEscalated.get();
    }
  }
}
