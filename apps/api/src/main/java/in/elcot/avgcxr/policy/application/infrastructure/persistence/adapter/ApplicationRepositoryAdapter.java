...
public class ApplicationRepositoryAdapter implements ApplicationRepositoryPort {
    ...
    @Override public Application save(Application a) { return ApplicationMapper.toDomain(jpa.save(ApplicationMapper.toEntity(a))); }
    @Override public Optional<Application> findById(UUID id) { return jpa.findById(id).map(ApplicationMapper::toDomain); }
    @Override public Page<Application> findAll(Pageable pageable) {
        return jpa.findAll(pageable).map(ApplicationMapper::toDomain);
    }
    @Override public Page<Application> findByApplicantId(UUID applicantId, Pageable p) {
