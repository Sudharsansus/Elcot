...
/**
 * Real implementation: persists to PostgreSQL via JPA, handles the
 * applicant lifecycle (DRAFT → SUBMITTED → UNDER_REVIEW → APPROVED/REJECTED).
 */
@Service
...
public class ApplicationUseCaseService
    ...
    @Override
    public Page<ApplicationResponse> findAll(String schemeId, String status, String district, Pageable pageable) {
        UUID sId = (schemeId == null || schemeId.isBlank()) ? null : UUID.fromString(schemeId);
        return appRepo.findAllFiltered(sId, status, district, pageable)
                .map(ApplicationUseCaseService::toResponse);
    }

    @Override
    public Page<ApplicationResponse> findMine(UUID applicantId, Pageable pageable) {
        return appRepo.findByApplicantId(applicantId, pageable).map(ApplicationUseCaseService::toResponse);
    }
    ...
    public Page<ApplicationResponse> findByCurrentUser(String status, Pageable pageable) {
        ...
        return appRepo.findByApplicantIdAndStatus(uid, status, pageable)
                .map(ApplicationUseCaseService::toResponse);
    }
    ...
    public ApplicationResponse submit(UUID id) {
        ...
        if (!"DRAFT".equals(e.getStatus())) {
            throw new IllegalStateException("Only DRAFT applications can be submitted (current: " + e.getStatus() + ")");
        }
