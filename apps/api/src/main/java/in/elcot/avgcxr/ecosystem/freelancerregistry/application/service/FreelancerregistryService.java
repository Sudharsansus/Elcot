...
import in.elcot.avgcxr.ecosystem.freelancerregistry.domain.exception.DuplicateFreelancerregistryException;
...
public class FreelancerregistryService implements CreateFreelancerregistryUseCase, GetFreelancerregistryUseCase {

    private final FreelancerregistryRepositoryPort repository;

    public FreelancerregistryService(FreelancerregistryRepositoryPort repository) { this.repository = repository; }

    @Override
    @Transactional
    public Freelancerregistry create(CreateFreelancerregistryCommand command) {
        Freelancerregistry entity = new Freelancerregistry(FreelancerregistryId.generate(), command.name());
        return repository.save(entity);
    }

    @Override
    public Optional<Freelancerregistry> findById(FreelancerregistryId id) { return repository.findById(id); }

    @Override
    public Freelancerregistry getById(FreelancerregistryId id) {
        return repository.findById(id).orElseThrow(() -> new FreelancerregistryNotFoundException(id.value()));
    }

    @Transactional
    public Freelancerregistry update(FreelancerregistryId id, UpdateFreelancerregistryCommand command) {
    ...
    @Transactional
    public void delete(FreelancerregistryId id) { repository.deleteById(id); }
}
