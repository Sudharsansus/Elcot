...
public class UserService implements CreateUserUseCase,
    ...
    public User create(CreateUserCommand cmd) {
        ...
        if (userRepo.existsByMobileNumber(cmd.mobileNumber())) throw new DuplicateUserException("mobile", cmd.mobileNumber());
        User user = new User(UserId.generate(), cmd.username(), cmd.email(), cmd.mobileNumber(), cmd.fullName(), cmd.roles());
        user.updateProfile(cmd.fullName(), cmd.fullNameTamil(), cmd.district());
        return userRepo.save(user);
    }

    // ---- in.elcot.avgcxr.platform.user.application.port.input.GetUserUseCase ----
    @Override public Optional<User> findById(UserId id) { return userRepo.findById(id); }
    @Override public User getById(UserId id) { return userRepo.findById(id).orElseThrow(() -> new UserNotFoundException(id.value())); }
    @Override public User getByUsername(String username) { return userRepo.findByUsername(username).orElseThrow(() -> new UserNotFoundException(username)); }

    @Transactional
    public User update(UserId id,
                       in.elcot.avgcxr.platform.user.application.command.UpdateUserCommand cmd) {
    ...
    public UserResponse getById(UUID id) {
        User u = userRepo.findById(UserId.of(id.toString()))
    ...
    public AuthResponse authenticate(String email, String password) {
        ...
        if (accountSecurity.isAccountLocked(user)) {
            ...
            throw new org.springframework.security.authentication.LockedException(
                    "Account is locked until " + user.getLockedUntil() + " due to too many failed login attempts");
        }
    ...
    public UserResponse update(UpdateProfileCommand command) {
        User u = userRepo.findById(UserId.of(command.userId().toString()))
    ...
    private String loadPasswordHash(String userId) {
        try {
            ...
            java.util.List<?> results = em.createQuery(
                    "SELECT u.passwordHash FROM UserEntity u WHERE u.id = :id")
    ...
    private static UserResponse toUserResponse(User u) {
        return new UserResponse(
                ...
                u.getLastLoginAt() != null ? u.getLastLoginAt().atZone(java.time.ZoneId.systemDefault()).toLocalDateTime() : null,
