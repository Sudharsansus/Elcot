...
public class SchemeWorkflowHandler {
    ...
    public SchemeWorkflowHandler(RuntimeService runtimeService) { this.runtimeService = runtimeService; }

    public void startSchemeApprovalProcess(String schemeId) {
        log.info("Starting scheme approval workflow for: {}", schemeId);
        var variables = java.util.Map.of("schemeId", (Object) schemeId, "initiatedAt", java.time.Instant.now().toString());
