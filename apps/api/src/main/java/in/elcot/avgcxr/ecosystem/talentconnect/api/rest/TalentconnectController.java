...
public class TalentconnectController {
    ...
    public TalentconnectController(TalentconnectService service) { this.service = service; }

    @PreAuthorize("isAuthenticated()")
    @PostMapping
    public ResponseEntity<TalentconnectResponse> create(@Valid @RequestBody CreateTalentconnectRequest request) {
        Talentconnect entity = service.create(new CreateTalentconnectCommand(request.name(), request.description(), "system"));
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TalentconnectResponse> update(@PathVariable String id, @Valid @RequestBody UpdateTalentconnectRequest request) {
        Talentconnect entity = service.update(TalentconnectId.of(id), new UpdateTalentconnectCommand(request.name(), request.description()));
