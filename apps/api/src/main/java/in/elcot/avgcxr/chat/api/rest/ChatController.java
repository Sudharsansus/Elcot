 ...
 *
 * <p>Public endpoint -- works for anonymous users (sessionToken cookie)
 * and authenticated users (userId from JWT).</p>
 *
 * <p>Supports both REST (single response) and SSE (streamed response).
 * Bilingual: English and Tamil.</p>
 */
@RestController
...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<ChatTurnResponse>> send(
            ...
            Authentication auth) {
        UUID userId = (auth != null && auth.getName() != null)
                ? currentUserIdFromAuth(auth)
                : null;
        ChatTurnResult r = chatService.sendMessage(userId, req.sessionToken(), req.message(), req.language());
    ...
    public ResponseEntity<ApiResponse<ChatSessionResponse>> getSession(@PathVariable String token) {
        ChatSession s = sessionRepo.findBySessionToken(token)
                .orElseThrow(() -> new RuntimeException("Session not found"));
        List<ChatMessageResponse> msgs = messageRepo.findBySessionId(s.getId()).stream()
    ...
    public ResponseEntity<ApiResponse<List<ChatSessionResponse>>> listSessions(Authentication auth) {
        ...
        List<ChatSessionResponse> sessions = sessionRepo.findByUserId(userId).stream()
