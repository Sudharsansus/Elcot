...
/**
 * Real implementation: persists new users to PostgreSQL via UserRepositoryPort, hashes the password
 * with BCrypt, and mints a JWT for immediate login.
 */
@Service
@Transactional
...
public class CreateUserUseCaseService implements CreateUserUseCase {
  ...
  public AuthResponse create(RegisterUserCommand cmd) {
    ...
    User user =
        new User(UserId.generate(), username, cmd.email(), cmd.phone(), cmd.fullName(), roles);
    ...
    if (consentsToRecord == null || consentsToRecord.isEmpty()) {
      consentsToRecord =
          java.util.Map.of(AccountSecurityService.CONSENT_DPDP_DATA_PROCESSING, true);
    }
    accountSecurity.recordConsents(userId, consentsToRecord, cmd.ipAddress(), cmd.userAgent());

    log.info(
        "User registered: id={} username={} consents={}",
        userId,
        username,
        consentsToRecord.keySet());
