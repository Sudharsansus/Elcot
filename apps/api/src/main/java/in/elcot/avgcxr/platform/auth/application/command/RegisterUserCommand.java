 ...
 *
 * <p>Previously named CreateUserCommand which collided with the
 * {@code platform.user.application.command.CreateUserCommand} (used by
 * admin-style user creation in /api/v1/users). Renamed for clarity per
 * audit fix MEDIUM-014. Both records still exist with their respective
 * purposes; only the names differ.</p>
 */
public record RegisterUserCommand(
    ...
    public static RegisterUserCommand from(CreateUserRequest request) {
        return new RegisterUserCommand(
                ...
                null, null, null  // consents + IP + UA populated at controller layer
        );
    }
}