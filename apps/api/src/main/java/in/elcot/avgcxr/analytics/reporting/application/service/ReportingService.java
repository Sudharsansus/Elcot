...
public class ReportingService implements GetReportingUseCase {
    ...
    public ReportingService(ReportingRepositoryPort repo) { this.repo = repo; }

    @Override
    public ReportingResponse generateApplicationReport(LocalDate from, LocalDate to, String district) {
    ...
    public ReportingResponse generateDisbursementReport(LocalDate from, LocalDate to) {
        ...
        return new ReportingResponse("DISBURSEMENT", "Disbursement Report", rows, Map.of("totalDisbursed", 0));
    }
    ...
    public byte[] exportApplicationReportCsv(LocalDate from, LocalDate to) {
        ...
        try (PrintWriter pw = new PrintWriter(baos, true, StandardCharsets.UTF_8)) {
            ...
            for (Map<String, Object> row : rows) {
                pw.printf("%s,%s,%s,%s,%s,%s,%s%n", row.getOrDefault("app_number",""), row.getOrDefault("scheme",""), row.getOrDefault("applicant",""), row.getOrDefault("district",""), row.getOrDefault("status",""), row.getOrDefault("submitted",""), row.getOrDefault("amount",""));
            }
        }
