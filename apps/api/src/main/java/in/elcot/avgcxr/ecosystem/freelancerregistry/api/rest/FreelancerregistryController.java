...
public class FreelancerregistryController {
    ...
    public FreelancerregistryController(FreelancerregistryService service) { this.service = service; }

    @PreAuthorize("isAuthenticated()")
    @PostMapping
    public ResponseEntity<FreelancerregistryResponse> create(@Valid @RequestBody CreateFreelancerregistryRequest request) {
        Freelancerregistry entity = service.create(new CreateFreelancerregistryCommand(request.name(), request.description(), "system"));
    ...
    public ResponseEntity<FreelancerregistryResponse> getById(@PathVariable String id) {
        return ResponseEntity.ok(FreelancerregistryResponse.from(service.getById(FreelancerregistryId.of(id))));
    }

    @PreAuthorize("hasAnyRole('ADMIN','NODAL_OFFICER')")
    @PutMapping("/{id}")
    public ResponseEntity<FreelancerregistryResponse> update(@PathVariable String id, @Valid @RequestBody UpdateFreelancerregistryRequest request) {
        Freelancerregistry entity = service.update(FreelancerregistryId.of(id), new UpdateFreelancerregistryCommand(request.name(), request.description()));
