...
public interface JpaReferenceDataRepository extends JpaRepository<ReferenceDataEntity, UUID> {
  ...
  @Query(
      "SELECT r FROM ReferenceDataEntity r WHERE r.category = :category AND r.isActive = true ORDER BY r.sortOrder, r.name")
  List<ReferenceDataEntity> findByCategoryActive(@Param("category") String category);

  @Query(
      "SELECT r FROM ReferenceDataEntity r WHERE r.category = :category ORDER BY r.sortOrder, r.name")
