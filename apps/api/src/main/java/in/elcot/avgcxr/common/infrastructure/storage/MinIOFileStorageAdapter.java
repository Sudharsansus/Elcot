...
/**
 * MinIO-backed implementation of FileStoragePort.
 * Hexagonal boundary: application layer depends only on the port.
 */
@Component
public class MinIOFileStorageAdapter implements FileStoragePort {
    ...
    public MinIOFileStorageAdapter(
            ...
            @Value("${minio.bucket:avgcxr-files}") String bucket) {
    ...
    public String upload(String objectName, byte[] content, String contentType) {
        try {
            ...
            log.info("Uploaded object {} to MinIO bucket {} ({} bytes)", objectName, bucket, content.length);
            return objectName;
        } catch (Exception e) {
