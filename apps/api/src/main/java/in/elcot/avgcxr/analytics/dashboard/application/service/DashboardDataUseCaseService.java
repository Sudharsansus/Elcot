 ...
 *
 * <p>HEXAGONAL FIX (HIGH-005 audit): Application layer depends ONLY on the output port. No JPA /
...
@Slf4j
public class DashboardDataUseCaseService
    implements CreateDashboardDataUseCase, GetDashboardDataUseCase {
  ...
  public DashboardDataResponse getById(UUID id) {
    DashboardData data =
        repo.findById(id).orElseThrow(() -> new DashboardDataNotFoundException(id));
