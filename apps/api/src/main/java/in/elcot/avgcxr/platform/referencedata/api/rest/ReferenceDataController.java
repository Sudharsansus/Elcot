...
/**
 * Public reference-data endpoints. Exposes Tamil Nadu's 38 districts,
 * AVGC subspecialties, and any custom reference data seeded in the
 * {@code reference_data} table.
 *
 * <p>Closes Gap 2 of the audit: the {@code districts}, {@code subspecialties}
 * and {@code reference_data} tables now have JPA entities AND are read
...
public class ReferenceDataController {
    ...
    public ReferenceDataController(JpaDistrictRepository districtRepo,
    ...
    public ApiResponse<List<Map<String, String>>> districts() {
        List<Map<String, String>> list = districtRepo.findByOrderByNameEnAsc().stream()
                .map(d -> Map.of(
                        "code", d.getCode(),
                        "nameEn", d.getNameEn(),
                        "nameTa", d.getNameTa()
    ...
    public ApiResponse<List<Map<String, String>>> subspecialties() {
        List<Map<String, String>> list = subspecialtyRepo.findByOrderByNameEnAsc().stream()
                .map(s -> Map.of(
                        "code", s.getCode(),
                        "nameEn", s.getNameEn(),
                        "nameTa", s.getNameTa()
    ...
    public ApiResponse<List<Map<String, Object>>> byCategory(@PathVariable String category) {
        List<Map<String, Object>> list = refDataRepo.findByCategoryActive(category).stream()
                .map(r -> {
