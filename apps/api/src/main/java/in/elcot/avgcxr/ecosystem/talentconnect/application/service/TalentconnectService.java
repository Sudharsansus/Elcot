...
import in.elcot.avgcxr.ecosystem.talentconnect.domain.exception.DuplicateTalentconnectException;
...
public class TalentconnectService implements CreateTalentconnectUseCase, GetTalentconnectUseCase {
    ...
    public TalentconnectService(TalentconnectRepositoryPort repository) { this.repository = repository; }
    ...
    @Override
    public Optional<Talentconnect> findById(TalentconnectId id) { return repository.findById(id); }

    @Override
    public Talentconnect getById(TalentconnectId id) {
        return repository.findById(id).orElseThrow(() -> new TalentconnectNotFoundException(id.value()));
    }
    ...
    @Transactional
    public void delete(TalentconnectId id) { repository.deleteById(id); }
}
