...
public class AuditRepositoryAdapter implements AuditRepositoryPort {
    ...
    @Override
    public Audit save(Audit entity) { return mapper.toDomain(jpaRepo.save(mapper.toEntity(entity))); }

    @Override
    public Optional<Audit> findById(AuditId id) { return jpaRepo.findById(id.toString()).map(mapper::toDomain); }

    @Override
    public void deleteById(AuditId id) { jpaRepo.deleteById(id.toString()); }

    @Override
    public boolean existsById(AuditId id) { return jpaRepo.existsById(id.toString()); }
}
