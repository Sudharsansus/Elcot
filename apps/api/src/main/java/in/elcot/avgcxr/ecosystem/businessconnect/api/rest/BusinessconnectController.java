...
public class BusinessconnectController {
  ...
  public BusinessconnectController(BusinessconnectService service) {
    this.service = service;
  }

  @PreAuthorize("isAuthenticated()")
  @PostMapping
  public ResponseEntity<BusinessconnectResponse> create(
      @Valid @RequestBody CreateBusinessconnectRequest request) {
    Businessconnect entity =
        service.create(
            new CreateBusinessconnectCommand(request.name(), request.description(), "system"));
  ...
  public ResponseEntity<BusinessconnectResponse> getById(@PathVariable String id) {
    return ResponseEntity.ok(
        BusinessconnectResponse.from(service.getById(BusinessconnectId.of(id))));
  }

  @PreAuthorize("hasAnyRole('ADMIN','NODAL_OFFICER')")
  @PutMapping("/{id}")
  public ResponseEntity<BusinessconnectResponse> update(
      @PathVariable String id, @Valid @RequestBody UpdateBusinessconnectRequest request) {
    Businessconnect entity =
        service.update(
            BusinessconnectId.of(id),
            new UpdateBusinessconnectCommand(request.name(), request.description()));
