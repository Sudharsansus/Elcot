...
public class TalentconnectRepositoryAdapter implements TalentconnectRepositoryPort {
    ...
    public TalentconnectRepositoryAdapter(JpaTalentconnectRepository jpaRepo, TalentconnectMapper mapper) {
        this.jpaRepo = jpaRepo;
        this.mapper = mapper;
    }

    @Override
    public Talentconnect save(Talentconnect entity) { return mapper.toDomain(jpaRepo.save(mapper.toEntity(entity))); }

    @Override
    public Optional<Talentconnect> findById(TalentconnectId id) { return jpaRepo.findById(id.toString()).map(mapper::toDomain); }

    @Override
    public void deleteById(TalentconnectId id) { jpaRepo.deleteById(id.toString()); }

    @Override
    public boolean existsById(TalentconnectId id) { return jpaRepo.existsById(id.toString()); }
}
