 ...
 *
 * <p>Used for ADMIN, NODAL_OFFICER, and DISTRICT_OFFICER roles per tender requirements.
 */
@Service
public class MfaService {
  ...
  /**
   * Generate a new TOTP secret + QR code URL for the user. The user scans the QR code with Google
  ...
  public byte[] generateQrCodePng(String otpAuthUrl, int width, int height) {
    try {
      ...
      var matrix =
          new QRCodeWriter().encode(otpAuthUrl, BarcodeFormat.QR_CODE, width, height, hints);
  ...
  /**
   * Verify a 6-digit TOTP code against the user's secret. Allows ±1 time step window (so codes work
