...
public record ApiResponse<T>(
    boolean success, T data, ApiError error, String message, LocalDateTime timestamp) {
  ...
  public static <T> ApiResponse<T> error(
      String code, String message, java.util.List<ApiError> details) {
    return new ApiResponse<>(
        false, null, ApiError.of(code, message, details), null, LocalDateTime.now());
  }
  ...
  public static <T> ApiResponse<T> paginated(T data, int page, int size, long total) {
    return new ApiResponse<>(
        ...
        null,
        String.format(
            "Page %d of %d (total: %d)", page, (int) Math.ceil((double) total / size), total),
        LocalDateTime.now());
  }
}
