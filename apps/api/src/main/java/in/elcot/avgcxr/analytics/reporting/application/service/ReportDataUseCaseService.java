 ...
 *
 * <p>HEXAGONAL FIX (HIGH-004 audit): Application layer depends ONLY on the output port.
...
public class ReportDataUseCaseService implements CreateReportDataUseCase, GetReportDataUseCase {
    ...
    public ReportDataResponse getById(UUID id) {
        ReportData data = repo.findById(id)
                .orElseThrow(() -> new ReportDataNotFoundException(id));
