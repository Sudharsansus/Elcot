 ...
 *
 * <p>The chat agent responds in Tamil when the user's input contains Tamil script (Unicode range
 * U+0B80-U+0BFF) OR when the session language is set to 'ta'.
 */
@Service
public class TamilSupport {
  ...
  public String detectLanguage(String userInput, String sessionLanguage) {
    ...
    if (sessionLanguage != null
        && (sessionLanguage.equalsIgnoreCase("ta") || sessionLanguage.equalsIgnoreCase("tamil"))) {
      return "ta";
    }
    return "en";
  }
  ...
  /**
   * Quick translation of common portal terms (en → ta). For full translation, the LLM does it; this
   * is a fallback for the rule-based provider and for snippets in the UI.
   */
