...
public interface JpaChatMessageRepository extends JpaRepository<ChatMessageEntity, UUID> {
  ...
  @Query(
      "SELECT m FROM ChatMessageEntity m WHERE m.sessionId = :sessionId ORDER BY m.createdAt ASC")
  ...
  @Query(
      "SELECT m FROM ChatMessageEntity m WHERE m.sessionId = :sessionId ORDER BY m.createdAt DESC LIMIT :limit")
