...
/**
 * Real implementation: persists to PostgreSQL via JPA, supports listing with category/status/search
 * filters, and admin create + status update.
 */
@Service
...
public class SchemeUseCaseService implements CreateSchemeUseCase, GetSchemeUseCase {
  ...
  public SchemeResponse updateStatus(UUID id, String newStatus) {
    ...
    SchemeEntity entity =
        schemeRepo.findById(id).orElseThrow(() -> new SchemeNotFoundException(id));
  ...
  @Override
  public Page<SchemeResponse> findAll(
      String category, String status, String search, Pageable pageable) {
