...
public interface JpaApplicationRepository extends JpaRepository<ApplicationEntity, UUID> {
  ...
  Page<ApplicationEntity> findByApplicantIdAndStatus(
      UUID applicantId, String status, Pageable pageable);

  @Query(
      "SELECT a FROM ApplicationEntity a WHERE "
          + "(:schemeId IS NULL OR a.schemeId = :schemeId) AND "
          + "(:status IS NULL OR a.status = :status) AND "
          + "(:district IS NULL OR a.district = :district) "
          + "ORDER BY a.createdAt DESC")
  Page<ApplicationEntity> findAllFiltered(
