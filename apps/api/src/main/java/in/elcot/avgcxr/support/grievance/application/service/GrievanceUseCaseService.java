 ...
 *
 * <p>HEXAGONAL FIX (HIGH-002 audit): Application layer depends ONLY on the output port interface in
...
public class GrievanceUseCaseService implements CreateGrievanceUseCase, GetGrievanceUseCase {
  ...
  public GrievanceResponse getById(UUID id) {
    Grievance grievance = repo.findById(id).orElseThrow(() -> new GrievanceNotFoundException(id));
    return new GrievanceResponse(
        grievance.getId(), grievance.getCreatedAt(), grievance.getUpdatedAt());
  }
