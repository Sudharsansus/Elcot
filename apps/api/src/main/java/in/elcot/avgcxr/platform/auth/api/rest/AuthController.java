...
public class AuthController {
  ...
  private final in.elcot.avgcxr.platform.user.application.service.AccountSecurityService
  ...
  @Operation(
      summary = "Log in",
      description = "Authenticates by email + password and returns access/refresh tokens.")
  @PostMapping("/login")
  ...
  public ResponseEntity<ApiResponse<MfaService.MfaEnrollment>> setupMfa() {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(enrollment, "Scan QR with Google Authenticator then call /mfa/verify"));
  }
  ...
  public ResponseEntity<byte[]> qrPng() {
    ...
    return ResponseEntity.ok().header(HttpHeaders.CONTENT_TYPE, "image/png").body(png);
  }
  ...
  public ResponseEntity<ApiResponse<java.util.Map<String, Object>>> verifyMfa(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            java.util.Map.of("valid", valid), valid ? "MFA verified" : "Invalid or expired code"));
  }
  ...
  public ResponseEntity<
          ApiResponse<java.util.List<in.elcot.avgcxr.platform.user.domain.model.UserConsent>>>
      myConsents() {
  ...
  /** Record a consent decision. Body: { "consentType": "DPDP_DATA_PROCESSING", "granted": true } */
  @PostMapping("/me/consents")
  @PreAuthorize("isAuthenticated()")
  public ResponseEntity<ApiResponse<in.elcot.avgcxr.platform.user.domain.model.UserConsent>>
      recordConsent(
          @RequestBody java.util.Map<String, Object> body, HttpServletRequest httpRequest) {
    ...
    if (consentType == null || consentType.isBlank()) {
      return ResponseEntity.badRequest()
          .body(ApiResponse.error("VALIDATION_FAILED", "consentType is required"));
    }
    var user = getUserUseCase.getCurrentUser();
    var consent =
        accountSecurityService.recordConsent(
            user.id(),
            consentType,
            granted,
            extractIp(httpRequest),
